...
| `supabase/migration_005_adaptive_watering.sql` | Adds `care_tasks.adjust_factor` / `last_checked_at` / `last_status` for moisture-driven adaptive watering, and pulls existing water tasks in to re-check within ~2 days. **NOT YET run on live project — `getDueTasks` selects `last_status`, so the Today screen 500s until it's run.** Idempotent. Already folded into `schema.sql` for fresh installs. | 🟡 **user action needed** |
